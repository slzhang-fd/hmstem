...
#' @export hiera_1factor_gibbs
hiera_1factor_gibbs <- function(Y, x_covs, i_ind, max_steps, cor_step_size = 0.01){
  K <- ncol(Y)
  rcd_num <- nrow(Y)
  ind_num <- max(i_ind)
  cov_num <- ncol(x_covs)
...
#' @export res_summary_1fac_gibbs
res_summary_1fac_gibbs <- function(hiera_res, xcov, burnin){
...
#' @export hiera_1factor_Gibbs_addon
hiera_1factor_Gibbs_addon <- function(Y, x_covs, i_ind, max_steps, res, cor_step_size = 0.02){
  K <- ncol(Y)
  rcd_num <- nrow(Y)
  ind_num <- max(i_ind)
  cov_num <- ncol(x_covs)
