...
#' @export res_summary_3fac_gibbs
res_summary_3fac_gibbs <- function(hiera_res, xcov, burnin){
  K <- sqrt(ncol(hiera_res[[2]]))
  p <- ncol(hiera_res[[1]]) / K
  stem_len <- nrow(hiera_res[[2]])
  ...
  process_names <- NULL
  if( K == 4 ) process_names <- c('tcprac', 'tcfin', 'fcprac', 'fcfin')
  CORR_u_all <- CORR_v_all <- CORR_w_all <- matrix(0, stem_len - burnin, 16)
  for(iter in (burnin+1):stem_len){
    CORR_u_all[iter-burnin,] <- cov2corr(hiera_res$Sigma_u_all[iter,])
    CORR_v_all[iter-burnin,] <- cov2corr(hiera_res$Sigma_v_all[iter,])
    CORR_w_all[iter-burnin,] <- cov2corr(hiera_res$Sigma_w_all[iter,])
  }

  return(c(summary_res_basic(hiera_res$coeffs_all[-(1:burnin),], 'coeffs', p, K, list(cov_names,process_names)),
           summary_res_basic(hiera_res$Sigma_e_all[-(1:burnin),], 'Sigma_e', K, K, list(process_names,process_names)),
           summary_res_basic(hiera_res$Sigma_u_all[-(1:burnin),], 'Sigma_u', K, K, list(process_names,process_names)),
           summary_res_basic(hiera_res$Sigma_v_all[-(1:burnin),], 'Sigma_v', K, K, list(process_names,process_names)),
           summary_res_basic(hiera_res$Sigma_w_all[-(1:burnin),], 'Sigma_w', K, K, list(process_names,process_names)),
           summary_res_basic(CORR_u_all, 'CORR_u', K, K, list(process_names,process_names)),
           summary_res_basic(CORR_v_all, 'CORR_v', K, K, list(process_names,process_names)),
           summary_res_basic(CORR_w_all, 'CORR_w', K, K, list(process_names,process_names))))
}
